...
import CashReceipt from '@/components/orders/CashReceipt'

export default function Orders() {
  const sampleOrders = [
  { id: 'ORD001', customer: 'Aman Sharma', address: 'Bandra, Mumbai', items: [{ name: 'Biryani', qty: 1 }], total: 12.99, status: 'preparing', partner: { name: 'Ravi Kumar', status: 'available' as const } },
  { id: 'ORD002', customer: 'Priya Patel', address: 'Connaught Place, Delhi', items: [{ name: 'Butter Chicken', qty: 1 }], total: 14.5, status: 'out-for-delivery', partner: { name: 'Kavita Mishra', status: 'busy' as const } },
    { id: 'ORD003', customer: 'Rohit Singh', address: 'Koramangala, Bengaluru', items: [{ name: 'Masala Dosa', qty: 2 }], total: 8.0, status: 'accepted', partner: null },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6">
          ...
          <div className="mt-4">
            {/* show a small cash receipt preview for the first order as an example */}
            <CashReceipt order={{
              id: sampleOrders[0].id,
              customer: sampleOrders[0].customer,
              items: sampleOrders[0].items?.map(it => ({ name: it.name, qty: it.qty, price: (it.name.toLowerCase().includes('biryani') ? 250 : 120) })),
              subtotal: 250,
              deliveryFee: 35,
              surge: 1.0,
              total: 285,
